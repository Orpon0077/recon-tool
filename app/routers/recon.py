...
async def api_full_scan(payload: ScanRequest) -> dict:
    ...
    try:
        ...
        # ── Helper: run sync functions in thread with timeout ──
        ...
        # ── Helper: run async functions with timeout ──
        ...
        # ── All modules as tasks with increased timeouts ──
        ssl_task = run_sync_with_timeout(analyze_ssl, url, timeout=30)
        security_task = run_sync_with_timeout(analyze_security_headers, url, timeout=30)
        firewall_task = run_sync_with_timeout(detect_firewall, url, timeout=30)
        tech_task = run_sync_with_timeout(detect_technologies, url, timeout=60)
        crawl_task = run_sync_with_timeout(crawl_website, url, timeout=900)          # 15 minutes for crawl
        subdomain_task = run_sync_with_timeout(discover_subdomains, url, timeout=400)
        js_task = run_sync_with_timeout(scan_javascript, url, timeout=60)
        ports_task = run_sync_with_timeout(
            scan_ports, url, payload.port_option, payload.custom_ports,
            timeout=90
        )
        screenshot_task = run_async_with_timeout(capture_screenshot, url, timeout=120)

        # ── Gather all modules with a global timeout of 40 minutes ──
        try:
            ssl_result, security_result, firewall_result, tech_result, crawl_result, subdomain_result, js_result, ports_result = await asyncio.wait_for(
                asyncio.gather(
                    ssl_task,
                    security_task,
                    firewall_task,
            ...
            return {
                "url": url,
                "error": "Overall scan timed out after 40 minutes. Partial results may be available.",
                "scan_id": None
            }
